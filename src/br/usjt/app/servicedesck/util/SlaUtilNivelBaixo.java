...
public class SlaUtilNivelBaixo extends SlaUtil {
	 ...
	 * @author Hygor S.
	 * 
	 * 	 Calcula a horas passadas do registro do chamado com a contabilidade da sla de 12 hrs x 5
	 * 
	...
	protected  StatusChamado contabilizar(Chamado chamado, Calendar calendar) {
		...
		if (!dataDoChamado.equals(dataDeHoje)) {
			...
			if (calendar.get(Calendar.HOUR_OF_DAY) >= min && calendar.get(Calendar.HOUR_OF_DAY) <= max && !isFimdeSemana) {
				hr += calendar.get(Calendar.HOUR_OF_DAY) - min;
			...
			if ((data.get(Calendar.HOUR_OF_DAY) >= min && data.get(Calendar.HOUR_OF_DAY) <= max && !isFimdeSemana)
				...
				hr = calendar.get(Calendar.HOUR_OF_DAY) - data.get(Calendar.HOUR_OF_DAY);
				
			} else if((data.get(Calendar.HOUR_OF_DAY) < min && calendar.get(Calendar.HOUR_OF_DAY) <= max && !isFimdeSemana)){
				hr = calendar.get(Calendar.HOUR_OF_DAY) - min;
			} else if((data.get(Calendar.HOUR_OF_DAY) >= min && calendar.get(Calendar.HOUR_OF_DAY) >= max && !isFimdeSemana)){
				hr = max - data.get(Calendar.HOUR_OF_DAY);
			} 
		...
		int hrsUteis = diasUteis * (max - min) + hr;
...
//		System.out.println("horas uteis que passaram " + hrsUteis);
//		System.out.println(chamado.getStatus());
	...
	/**
	 * @author Hygor S.
	 * @param data Representa uma data para verificar se ela  e sabado ou domindo
